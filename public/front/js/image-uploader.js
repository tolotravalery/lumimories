    ...
    $.fn.imageUploader = function (options) {
        ...
        plugin.init = function () {
            ...
            plugin.each(function (i, wrapper) {
                let $count = plugin.settings.preloaded.length !==0 ? plugin.settings.preloaded.length+1 : plugin.settings.count;
                // Create the container
                for(let j=0;j< $count ;j++){
                    let $container = createContainer(j);
                    ...
                    if (plugin.settings.preloaded.length > 0 && j > 0) {
                        // Change style
                        ...
                        $uploadedContainer.append(createImg(plugin.settings.preloaded[j-1].src, plugin.settings.preloaded[j-1].id, true));
                    }
                }
            });
        ...
        let createContainer = function (number) {
            // Create the image uploader container
            let $container = $('<div>', {class: 'image-uploader', id:'image-uploader-'+number});
            if(number > 0)  $container = $('<div>', {class: 'image-uploader image-uploader-dotted', id:'image-uploader-'+number});
            ...
            let $uploadedContainer = $('<div>', {class: 'uploaded'}).appendTo($container),
                ...
                $origin = $origin.split("/edit-profil");
                if($origin.length > 1){
                    $origin = $origin[0];
                }
                else{
                    $origin   = window.location.href;
                    $origin = $origin.split("/ajout-profil");
                    if($origin.length > 1){
                        $origin = $origin[0];
                    }
                    else{
                        $origin   = window.location.href;
                        $origin = $origin.split("/detail");
                        $origin = $origin[0];
                    }
                }
                $i = '';
                if(number === 0) $i = $('<img src="'+$origin+'/public/front/img/add-photo.png">').appendTo($textContainer);
                else{
                    //$i = $('<i>', {class: 'fa fa-camera'}).appendTo($textContainer);
                    $i = $('<img src="'+$origin+'/public/front/img/add-photo-simple.png">').appendTo($textContainer);
                }
                // Create the text and append it to the text container
                $span = "";
                if(number === 0){
                    $span = $('<span>', {text: plugin.settings.labelFirst}).appendTo($textContainer);
                }
                else{
                    $span = $('<span>', {text: plugin.settings.label+number}).appendTo($textContainer);
                }
        ...
        let contentResponse = function(parent, content, n){
            let $count = plugin.settings.preloaded.length !==0 ? plugin.settings.preloaded.length+1 : plugin.settings.count;
            content = $('.'+parent[0].classList[0]+' > ' +'#image-uploader-'+n);
            while(n <= $count){
                if(content[0].className === "image-uploader image-uploader-dotted has-files" || content[0].className === "image-uploader has-files"){
                    n = n + 1;
                    content = $('.'+parent[0].classList[0]+' > ' +'#image-uploader-'+n);
                    contentResponse(content, n);
                }
                else {
                    content = $('.'+parent[0].classList[0]+' > ' +'#image-uploader-'+n);
                    break;
        ...
        let fileSelectHandler = function (e) {
            ...
            let $parent = $container.parent();
                let $n = 1 ;
                $container = contentResponse($parent, $('.'+$parent[0].classList[0]+' > '+'#image-uploader-'+$n), $n);
            ...
            if (validFiles.length) {
                if($container.hasClass("has-files") === false){
                    // Change the container style
